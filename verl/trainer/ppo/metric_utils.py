...
def prepare_validation_infos_for_metrics(
    data_sources: list[str] | np.ndarray,
    infos_dict: dict[str, list[Any]],
) -> dict[str, list[Any]]:
    """Attach a prediction field for majority-vote bootstrap metrics when possible."""
    prepared = dict(infos_dict)
    if "pred" in prepared:
        return prepared

    if "extracted_answer" in prepared:
        prepared["pred"] = list(prepared["extracted_answer"])
        return prepared

    return prepared


def classify_validation_metric_section(
    var_name: str, core_var: str, metric_name: str, n_max: int
) -> str:
    """Classify validation metrics into the existing logging sections."""
    if metric_name.startswith("maj@"):
        return "val-aux"

    if var_name == core_var and (f"@{n_max}" in metric_name):
        if metric_name.startswith("mean"):
            return "mean_accuracies"
        if metric_name.startswith("best") and "std" not in metric_name:
            return f"pass@{n_max}_accuracies"
        return "val-core"

    return "val-aux"


def normalize_validation_metric_name(metric_name: str) -> str:
    """Map internal bootstrap metric names to user-facing logged names."""
    if metric_name.startswith("maj@"):
        return metric_name.replace("maj@", "major@", 1)
    return metric_name
...
def process_validation_metrics(
    ...
    processed_prompts = 0
    print(
        f"[process_validation_metrics] Starting bootstrap for {total_prompts} prompts across {len(data_src2prompt2var2vals)} data sources"
    )
    ...
    for ds_idx, (data_source, prompt2var2vals) in enumerate(
        data_src2prompt2var2vals.items()
    ):
        ds_num_prompts = len(prompt2var2vals)
        print(
            f"[process_validation_metrics] Processing data source {ds_idx+1}/{len(data_src2prompt2var2vals)}: {data_source} ({ds_num_prompts} prompts)"
        )
        for prompt_idx, (prompt, var2vals) in enumerate(prompt2var2vals.items()):
            ...
            if processed_prompts % 100 == 0 or processed_prompts == total_prompts:
                print(
                    f"[process_validation_metrics] Processed {processed_prompts}/{total_prompts} prompts ({100*processed_prompts/total_prompts:.1f}%)"
                )
