...
def _state_dict_uses_dtensor(state_dict) -> bool:
    # INTERFERENCE_ESTIMATE_COMPAT:
    # Maze interference-estimate reads actor FSDP shards outside the original
    # training worker stack. Some saved checkpoints store DTensor shards rather
    # than ShardedTensor shards, so load-time state-dict flavor has to be
    # inferred from the serialized payload.
    if isinstance(state_dict, dict):
        values = state_dict.values()
    elif isinstance(state_dict, (list, tuple)):
        values = state_dict
    else:
        return isinstance(state_dict, DTensor)

    for value in values:
        if isinstance(value, DTensor):
            return True
        if isinstance(value, (dict, list, tuple)) and _state_dict_uses_dtensor(value):
            return True
    return False
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        # INTERFERENCE_ESTIMATE_COMPAT:
        # Detect whether the serialized sharded checkpoint uses DTensor or
        # ShardedTensor and mirror that choice in the FSDP load context. This
        # keeps regular FSDP resume behavior intact while allowing the maze
        # interference-estimate offline loader to reuse the same checkpoint
        # manager.
        ...
        if self.should_load_model:
            ...
            use_dtensor = use_dtensor or _state_dict_uses_dtensor(model_state_dict)

        if self.should_load_optimizer:
            remote_optim_path = os.path.join(local_path, f"optim_world_size_{self.world_size}_rank_{self.rank}.pt")
            local_optim_path = copy_to_local(remote_optim_path)
            optimizer_state_dict = torch.load(local_optim_path, map_location="cpu", weights_only=False)
            use_dtensor = use_dtensor or _state_dict_uses_dtensor(optimizer_state_dict)

        # every rank loads its own checkpoint shard with the matching state-dict flavor
        state_dict_cfg = (
            ShardedStateDictConfig(
                offload_to_cpu=True if is_cuda_available else False,
                _use_dtensor=use_dtensor,
            )
            if self.should_load_model
            else None
        )
