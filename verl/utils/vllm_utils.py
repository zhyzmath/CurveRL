...
def _patch_vllm_mig_device_id_mapping():
    """Patch vLLM CUDA device mapping to tolerate MIG UUID entries.

    Some Ray setups export CUDA_VISIBLE_DEVICES as MIG UUID strings such as
    ``MIG-GPU-<uuid>/0/0``. vLLM 0.8.x expects integer IDs and can raise
    ValueError during import-time capability checks. This patch maps MIG UUIDs
    back to their parent physical GPU index via NVML and falls back safely.
    """
    ...
    if getattr(vllm_cuda, "_verl_mig_patch_applied", False):
        return

    original = getattr(vllm_cuda, "device_id_to_physical_device_id", None)
    if original is None:
        platform_cls = getattr(vllm_cuda, "CudaPlatform", None)
        original = getattr(platform_cls, "device_id_to_physical_device_id", None) if platform_cls else None
    if original is None:
        vllm_cuda._verl_mig_patch_applied = True
        return

    def patched(device_id: int) -> int:
        ...
        raw_id = device_ids[device_id]
        try:
            return int(raw_id)
        except ValueError:
            if not raw_id.startswith("MIG-GPU-"):
                raise

            # raw_id example: MIG-GPU-<uuid>/0/0 -> parent UUID: GPU-<uuid>
            parent_uuid = raw_id.split("/", 1)[0].replace("MIG-", "", 1)

            try:
                device_count = vllm_cuda.pynvml.nvmlDeviceGetCount()
                for i in range(device_count):
                    handle = vllm_cuda.pynvml.nvmlDeviceGetHandleByIndex(i)
                    uuid = vllm_cuda.pynvml.nvmlDeviceGetUUID(handle)
            ...
            # Safe fallback for homogeneous GPU clusters.
            return device_id

    vllm_cuda.device_id_to_physical_device_id = patched
    vllm_cuda._verl_mig_patch_applied = True


_patch_vllm_mig_device_id_mapping()
...
def patch_vllm_moe_model_weight_loader(model):
    ...
    MLP_ATTR_MAPPING = {}
    if "MixtralForCausalLM" in globals():
