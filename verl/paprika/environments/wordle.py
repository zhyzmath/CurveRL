...
class WordleEnv(BaseEnv):
    ...
    custom_system_prompt_str = (
        "You are a helpful assistant. "
        "If you play Wordle, in the first turn you should choose the word SLATE. "
        "In the first turn, you should generate a step-by-step chain-of-thought, "
        "followed by <Answer> SLATE </Answer>."
    )
    ...
    def __init__(
        ...
        assert len(self.goal_state) ==  WORDLE_WORD_LENGTH
    ...
    def get_max_turns(self) -> int:
        return WORDLE_ENV_DATA["max_turns"]
    ...
    def step(
        ...
        try:
            ...
            return {
                "content": WORDLE_ENV_DATA["environment_default_response"],
                "goal_reached": False,
                "judge_label": False,
            }
        ...
        if len(last_word) != WORDLE_WORD_LENGTH:
            ...
            feedback += WORDLE_ENV_DATA["agent_optional_message"]
