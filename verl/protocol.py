...
def union_numpy_dict(tensor_dict1: dict[str, np.ndarray], tensor_dict2: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    # Keys that are allowed to be overwritten during union (e.g., generated data that may differ)
    overwritable_keys = {"finish_reasons"}
    
    for key, val in tensor_dict2.items():
        if key in tensor_dict1:
            assert isinstance(tensor_dict2[key], np.ndarray)
            assert isinstance(tensor_dict1[key], np.ndarray)
            # Skip equality check for keys that are expected to be overwritten
            if key not in overwritable_keys:
