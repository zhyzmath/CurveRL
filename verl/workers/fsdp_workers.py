...
def get_process_group_timeout() -> timedelta:
    timeout_seconds = int(os.getenv("VERL_PROCESS_GROUP_TIMEOUT_SECONDS", "1800"))
...
class ActorRolloutRefWorker(Worker, WorkerProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            ...
            torch.distributed.init_process_group(
                ...
                timeout=get_process_group_timeout(),
            )
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @WorkerProfiler.annotate(color="red")
    def generate_sequences(self, prompts: DataProto, **generation_kwargs):
        # Support all hardwares
...
class CriticWorker(Worker, WorkerProfilerExtension):
    def __init__(self, config):
        ...
        if not torch.distributed.is_initialized():
            torch.distributed.init_process_group(backend=get_nccl_backend(), timeout=get_process_group_timeout())
        self.config = config
