    ...
    def update_critic(self, data: DataProto):
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                # 添加tqdm进度条
                if world_size > 1:
                    desc = f"Critic[GPU {rank}/{world_size-1}] update_critic: {mini_batch_size} batches, {num_chunks} rounds"
                else:
                    desc = f"Critic update_critic: {mini_batch_size} batches, {num_chunks} rounds"
