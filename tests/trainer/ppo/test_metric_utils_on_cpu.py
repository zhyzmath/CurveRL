...
class TestValidationMetricHelpers(unittest.TestCase):
    """Tests for validation metric helper utilities."""

    def test_prepare_validation_infos_for_metrics_preserves_existing_pred(self):
        infos = {"reward": [1.0], "pred": ["A"]}
        prepared = prepare_validation_infos_for_metrics(
            data_sources=np.array(["openai/gsm8k"], dtype=object),
            infos_dict=infos,
        )
        self.assertEqual(prepared["pred"], ["A"])

    def test_prepare_validation_infos_for_metrics_promotes_extracted_answer(self):
        infos = {"reward": [1.0, 0.0], "extracted_answer": ["42", "7"]}
        prepared = prepare_validation_infos_for_metrics(
            data_sources=np.array(["openai/gsm8k", "openai/gsm8k"], dtype=object),
            infos_dict=infos,
        )
        self.assertEqual(prepared["pred"], ["42", "7"])

    def test_majority_metrics_are_logged_in_val_aux_with_major_name(self):
        self.assertEqual(
            classify_validation_metric_section("reward", "reward", "maj@256/mean", 256),
            "val-aux",
        )
        self.assertEqual(normalize_validation_metric_name("maj@8/std"), "major@8/std")
